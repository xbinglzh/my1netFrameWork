...
#define CCB_MEMBERVARIABLEASSIGNER_GLUE2(MEMBERVARIABLETYPE, MEMBERVARIABLE) \
    if (pTarget == this && 0 == strcmp(pMemberVariableName, (#MEMBERVARIABLE))) { \
        MEMBERVARIABLETYPE pOldVar = _##MEMBERVARIABLE; \
        _##MEMBERVARIABLE = dynamic_cast<MEMBERVARIABLETYPE>(pNode); \
        CC_ASSERT(_##MEMBERVARIABLE); \
        if (pOldVar != _##MEMBERVARIABLE) { \
            CC_SAFE_RELEASE(pOldVar); \
            _##MEMBERVARIABLE->retain(); \
        } \
        return true; \
    }
