...
#include "../utils/SPTweenFunction.h"
...
Tween::~Tween(void)
{
    CC_SAFE_RELEASE_NULL(m_pFrom);
    CC_SAFE_RELEASE_NULL(m_pTweenData);
    CC_SAFE_RELEASE_NULL(m_pBetween);
    CC_SAFE_RELEASE_NULL(m_pMovementBoneData);
}
...
void Tween::updateHandler()
{
    if (m_fCurrentPercent >= 1)
    {
        ...
        case SINGLE:
			...
			m_frameVisiable = m_pCurrentKeyFrame->m_visible;
            m_fCurrentPercent = 1;
            m_bIsComplete = true;
            break;
        case LIST_START:
