...
//        const GLchar* pszFragSource =str.c_str();
    ...
    CCGLProgram* ShaderManager::getShaderProgram(string shaderType){
...
CCGLProgram *ShaderManager::getColorShaderProgramOrCompile(const cocos2d::ccColor4F& color) {
    char key[20];
    sprintf(key, "%.3f%.3f%.3f", color.r, color.g, color.b);
    
    cocos2d::CCGLProgram* program = cocos2d::CCShaderCache::sharedShaderCache()->programForKey(key);
    if (!program) {
        char value[30];
...
//        { \n \
//        vec4 col = texture2D(u_texture, v_texCoord); \n \
//        gl_FragColor = " << value  << " * col.a * " << alpha << "  + col * " << value << " * col.a * (1.0 - " << alpha << "); \n\
//        }";
