...
#include "../utils/SPUtilMath.h"
#include "../utils/SPConstValue.h"
...
bool Animation::play(const char *_animationName, int _durationTo, int _durationTween,  int _loop, int _tweenEasing)
{
...
{
    const std::vector<std::string> & movName = m_pAnimationData->getMovNames();
    if (movName.size() > _animationIndex && _animationIndex >= 0) {
        const std::string _animationName = movName[_animationIndex];
        play(_animationName.c_str(),
             _durationTo,
             _durationTween,
             _loop,
             _tweenEasing);
        ...
        CCLOG("Animation::playByIndex Error: movName.size()=%d,_animationIndex=%d",movName.size(),_animationIndex);
    }
...
{
    //CCLOG("Animation::updateHandler %s",m_strMovementID.c_str());
    if (m_fCurrentPercent >= 1)
    {
        switch(m_iLoop)
        {
            ...
            case SINGLE:
                m_fCurrentPercent = 1;
                m_bIsComplete = true;
                //CCLOG("Animation: m_iLoop = %d COMPLETE = %s",m_iLoop , m_strMovementID.c_str());
    ...
    if (m_fCurrentPercent < 1 && m_fCurrentPercent > 0){
        MovementEventSignal.emit(m_pArmature, MOVEMENT_RUNNING, m_strMovementID.c_str());
    }
}
...
void Animation::updateMovementFrameData(float _currentPercent)
{
    ...
    if (!m_pCurrentFrameData ||
        _played >= m_pCurrentFrameData->m_iDuration + m_pCurrentFrameData->m_iStart ||
        _played < m_pCurrentFrameData->m_iStart) {
...
void Animation::editMovementByIndex(int _index)
{
    ...
    const std::string & _animationName = movName[_index];
