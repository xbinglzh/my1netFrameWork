...
class AnimNode : public cocos2d::CCNodeRGBA,
public AnimNodeDelegate,
public AnimationDelegateExt {
    
public:
    enum EAnimNodeTypeId {
		K_SPRITE_FILE  	  =0,//sprite
        K_SPRITE_FRAME    =1,//spriteframe
		K_CCBI_FILE 	  =2,//ccbi
        K_ARMATURE_FRAME  =3,//Armature spriteframe
	};
    
	AnimNode();
	virtual ~AnimNode();
    CREATE_FUNC(AnimNode);
    static AnimNode * createAnim(cocos2d::CCDictionary * dict,
                                 AnimNodeDelegate * delegate);
    
	const bool runAnimation(const std::string & name = "",
                            const float delay = 0.0f,
                            cocos2d::CCCallFuncO * calll = NULL);
    void  stopAnimation();
...
CREATE_NEW_INHERIT_CCNODELOADER_CLASS(AnimNode);
